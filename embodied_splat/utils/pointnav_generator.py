...
ISLAND_RADIUS_LIMIT = 2.0  # this is needed to ignore objects like bed
...
def get_largest_island_index(
    ...
    If no islands exist satisfying the indoor constraints, then the entire navmesh
    -1 is returned.
    """
...
def sample_navigable_point_from_largest_island(sim) -> List[float]:
    island_idx = get_largest_island_index(sim)
    return sim.pathfinder.get_random_navigable_point(
        max_tries=10, island_index=island_idx
    ).tolist()


def get_random_island_index(sim):
    return np.random.randint(0, sim.pathfinder.num_islands)


def sample_navigable_point_from_island_index(
    sim, island_index=0
) -> List[float]:
    return sim.pathfinder.get_random_navigable_point(
...
def is_compatible_episode(
    ...
    if (
        d_separation == np.inf
        or np.isnan(d_separation)
        or not np.isfinite(d_separation)
    ):
        return False, 0
...
def generate_pointnav_episode(
    ...
    while episode_count < num_episodes or num_episodes < 0:
        if island_sampling_type == "largest":
            target_position = sample_navigable_point_from_largest_island(sim)
        elif island_sampling_type == "random":
            random_island_index = get_random_island_index(sim)
            target_position = sample_navigable_point_from_island_index(
                sim, random_island_index
            )
        else:
        ...
        for _retry in range(number_retries_per_target):
            if island_sampling_type == "largest":
                source_position = sample_navigable_point_from_largest_island(
                    sim
                )
            elif island_sampling_type == "random":
                source_position = sample_navigable_point_from_island_index(
                    sim, random_island_index
                )
            else:
                # HM3D generation - v0.8
        ...
        if is_compatible:
            ...
            episode_count += 1
            if not is_gen_shortest_path:
                yield episode
            else:
                yield episode, len(shortest_paths[0])
