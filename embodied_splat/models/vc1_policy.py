...
from embodied_splat.models.visual_encoders import Vc1Wrapper
...
class PointNavResNetVC1Policy(NetPolicy):
    def __init__(
        self,
    ...
    def from_config(
        ...
        **kwargs,
        ...
        # print(observation_space.keys())
        # for item, value in observation_space.items():
        #     print(item, " " if isinstance(value, spaces.Dict) else value.shape)
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        self.visual_keys = [
            ...
            if len(v.shape) > 1
            and k
            not in [ImageGoalSensor.cls_uuid, ImageGoalRotationSensor.cls_uuid]
        ]
        ...
        for uuid in [
            ...
            if uuid in observation_space.spaces:
                ...
                if backbone.startswith("vc1"):
                    model_id = backbone
                    goal_visual_encoder = Vc1Wrapper(
                        goal_observation_space, model_id=model_id
                    )
                    setattr(self, f"{uuid}_encoder", goal_visual_encoder)
                    goal_visual_fc = nn.Sequential(
                        nn.Linear(goal_visual_encoder.feats_size, hidden_size),
                        nn.ReLU(True),
                    )
                    setattr(self, f"{uuid}_fc", goal_visual_fc)
                elif backbone.startswith("resnet50_clip"):
                    goal_visual_encoder = ResNetCLIPEncoder(
                        (
                            goal_observation_space
                        ...
                        pooling=(
                            "avgpool" if "avgpool" in backbone else "attnpool"
                        ),
                    )
                    setattr(self, f"{uuid}_encoder", goal_visual_encoder)
                    ...
                    goal_visual_fc = nn.Sequential(
                        nn.Linear(
                            goal_visual_encoder.output_shape[0], hidden_size
                        ),
                        nn.ReLU(True),
                    )
                    setattr(self, f"{uuid}_fc", goal_visual_fc)
                    ...
                    goal_visual_fc = nn.Sequential(
                        nn.Flatten(),
                        nn.Linear(
                            np.prod(goal_visual_encoder.output_shape),
                            hidden_size,
                        ),
                        nn.ReLU(True),
                    )
